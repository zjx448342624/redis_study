...
6. 外网访问 ： 设置reids.conf 的配置
   ...
   2. 注释掉```bind ```
7. redis的密码设置：配置``` requirepass  foobared  ```
8. 白名单： 配置``` bind ip地址 ```
...
- 哨兵机制（sentinel)
  ...
  reids 通过哨兵机制去监控master和slave，如果master 挂掉以后会通过哨兵机制把其中一台slave节点升级为master，哨兵也可以做集群，进行哨兵之间的相互监控

  配置哨兵机制：

  ```
  sentinel monitor mymaster 192.168.177.131(masterip) 6379（master端口） 1（选举票数）
  ```

  哨兵集群只需要在多个配置文件中配置监控同一个master，这样哨兵之间就可见了，master挂掉以后哨兵机制的配置文件也会随之更改，更改为新选举的master的地址

- redis3.0以后的集群（高可用集群）

  1. 在redis3.0以前做集群是通过hash来做集群的，是通过应用层面来做的，通过key的值hash取模然后做到数据的分片

  2. 3.0以后集群的原理是通过是咯人（槽）的概念，在redis集群里面会有16384个槽，对于每个reids的键值进行CRC16算法，得到的结果再对16384进行取模，然后落到16384的槽中，redis的集群的每个节点分摊16384个节点如果有4个节点，那么就是16384/4，

  3. redis集群的节点新增以后：会从每个节点中那一部分数据到新加的节点，每个节点的数据需要通过手动迁移。

  4. reids集群的节点删除以后：就需要把自身的槽先迁移到其他的节点上，在删除以前必须要把数据进行迁移否则无法进行删除，迁移工作必须手动进行

     reids节点迁移可以通过```redis-trib.rb``` 进行操作

     如果节点奔溃了以后redis集群会进行进入fail状态

     reids集群至少需要6个节点3个master，3个slave，master和slave是1：1匹配的

- 已有集群方案

  - redis shardding  :而且jedis客户端支持shardding操作，配置多个地址，通过ip取模进行数据的分片通过redis的一致性hash来做的，增加和减少节点解决方案：pre shardding,可以通过预配置redis进行扩展（一开始有3台虚拟机，在3台虚拟机上配置9个reids,如果后期需要扩展，就可以把redis从3台机器扩展到9台机器上）
  - codis:是豌豆荚开源的一个reids集群策略，应用比较广泛，是通过应用层和客户端之间做代理的方式完成，是基于redis2.813开发出来的codis-server,codis也可以进行集群操作，如果后续进行增加节点删除节点codis会自己解决。
  - twemproxy：是通过应用层和客户端之间做代理的方式完成
...
- 缓存的更新
  1. 先删除缓存，在更新数据库（会出现一段时间的脏数据）
  2. 先更新数据库，在删除缓存（也会出现脏数据，但是概率比第一种低）
  3. 更新数据的时候，只更新缓存不更新数据库，然后通过异步调度批量更新数据库，直接操作内存，性能会提高
- 缓存击穿，缓存穿透，缓存失效
  - 缓存穿透：是指一些人恶意的使用一些不会存在的key去查询，然后每一次都会查询数据库，如果大量的数据请求，有可能会使数据库宕掉
    - 解决方案1： 对传递过来的key进行设置成特定的值，下次访问进来以后判断这个key的值，就不去访问数据库
    - 解决方案2：布隆过滤器，先把可能存在的数据内置起来，然后进行筛选
  - 缓存击穿：对于每天的热点数据的访问量比较大，如果某一时刻一批热点数据的过期时间正好到达，那么大量的数据请求就会到数据库，可能会使数据库宕机，
    - 解决方案1：互斥锁，通过redis设置一个互斥锁，这样的话就可以对大量请求进行排队操作，而且在设置key的失效时间的时候，尽量错开时间段
  - 缓存失效：缓存失效是和缓存击穿类似。也是同一时间段缓存大量失效导致的，因此尽量错开缓存的key的失效时间
